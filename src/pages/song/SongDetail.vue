      ...
      <div class="top-panel">
        ...
        <div class="info-right">
          ...
          <div class="button-row">
            ...
            <span class="btn" @click="addToList">
...
function getSourceUrl(){
  ...
  const sep = song.value.url.includes('?') ? '&' : '?'
  return song.value.url + sep + '_t=' + Date.now()
}

function goArtist(song) {
  if (song.singerUserId) { router.push(`/user-page/${song.singerUserId}`) }
  else if (song.singerId) { router.push(`/artist/${song.singerId}`) }
}
...
function addToList(){
  ...
  const item = { id: song.value.id, name: song.value.name, artist: song.value.singerName || song.value.singer || song.value.introduction || '未知', cover: getImage(song.value.pic), singerUserId: song.value.singerUserId, url: song.value.url && song.value.url.includes('://') ? song.value.url : attachImageUrl(song.value.url) }
...
async function toggleCommentLike(c){
  ...
  try{
    ...
    if(c.isLiked){ c.isLiked = false; c.likeCount = Math.max(0, (c.likeCount || 1) - 1) }
    else{ c.isLiked = true; c.likeCount = (c.likeCount || 0) + 1 }
...
function formatTime(time){
  ...
  const now = new Date()
  const diffMs = now - d
  const oh = 1000 * 60 * 60; const od = oh * 24
  if (diffMs >= 0 && diffMs < od) {
    const h = Math.floor(diffMs / oh); if (h >= 1) return h + '小时前'
    const m = Math.floor(diffMs / (1000 * 60)); if (m >= 1) return m + '分钟前'
    return '刚刚'
  }
...
<style scoped>
.song-detail{ padding: 24px; max-width: 1000px; margin: 0 auto; padding-bottom: 100px; }
...
.artist-link { cursor: pointer; color: #ec4141; }
...
.lyrics{ margin-top:16px; padding: 16px; background: #fafafa; border-radius: 8px; line-height:2; color:#444; }
