...
const handleAudit = async (row, status) => {
  try {
    ...
    ElMessage.error('审核失败：' + (e.response?.data?.message || e.message))
  }
}
...
const submitReject = async () => {
  ...
  try {
    const reason = rejectForm.value.auditReason
    await auditPost(rejectRow.value.id, 2, reason)
    rejectDialogVisible.value = false
    ElMessage.success('驳回成功')
    fetchData()
  } catch (e) {
    ElMessage.error('驳回失败：' + (e.response?.data?.message || e.message))
  }
}
