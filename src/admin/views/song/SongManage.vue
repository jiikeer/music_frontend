<template>
  <div class="page">
    <div class="toolbar">
      <el-input v-model="query" placeholder="请输入歌曲名" clearable style="width: 200px" @keyup.enter="handleSearch" />
    ...
    <el-table :data="tableData" border v-loading="loading">
      ...
      <el-table-column prop="collect_count" label="收藏量" width="80" />
...
import { getAdminSongPage, auditSong, adminDeleteSong } from '@/api'
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await getAdminSongPage(currentPage.value, pageSize.value, status.value === '' ? undefined : status.value)
...
const handleAudit = async (row, status) => {
  await auditSong({ id: row.id, status })
...
const submitReject = async () => {
  ...
  await auditSong({ id: rejectRow.value.id, status: 2, auditReason: rejectForm.value.auditReason })
...
const handleDelete = async (row) => {
  ...
  if (ok) { await adminDeleteSong(row.id); ElMessage.success('删除成功'); fetchData() }
}
